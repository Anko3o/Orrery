...
「心情幅度」读的是我们家的标签写法：tags 里有 `天气:落雪` / `轴:思念+8` 这种就算，没有就是 0，不影响别人使用。天气除放晴外一律 0.7（盖天气的规则本身就是「某根轴过了线」，下雪和雷暴一样重），轴按幅度除以满格。「实质改动次数」从 OB 3.2 的事件账本 `_ledger/events.jsonl` 数，账本不在就当 0。
